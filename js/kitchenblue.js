...
function isADefunctSite(sampleSite) {
  ...
  defunctDomains.forEach(function (item, index) {
...
function formatSearchResults(jsonResults) {
  ...
  if (jsonObject.results.length == 0) {
    ...
    jsonObject.results.forEach(function (item, index) {
      ...
      if (thumbnail == "") {
        thumbnail = "images/generic_dish.jpg";
      }

      const href = item.href;

      formatedText +=
        "<div class='dish-image-div'><a " +
        " href='" +
        href +
        "' target='_blank'><img class='dish-image' width='80' src='" +
        thumbnail +
        "' alt='recipe picture, link to recipe page'></a></div>";
      formatedText +=
        "<div " +
        "class='dish-title-div'><a href='" +
        href +
        "' target='_blank'>" +
        item.title +
        "</a></div>";
      formatedText +=
        "<div class='dish-ingredients-div'>Main ingredients: " +
        item.ingredients +
        "</div>";
    });
...
function setNotFoundMessages() {
  $("#search-results-heading").text(
    "No recipes found, please change search criteria."
  );
  $("#results").text("");
}
